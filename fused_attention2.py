...
def _attn_fwd(
    ...
    for rhidx in tl.static_range(loop_range1, loop_range1 *2):
        for rwidx in tl.static_range(loop_range2, loop_range2 *2):
            hidx = (rhidx - ((off_ht - h_offset) % loop_range1)) % loop_range1
            widx = (rwidx - ((off_wt - w_offset) % loop_range1)) % loop_range2
...
def _attn_bwd(Q, K, V, Bias, sm_scale,  #
    ...
    if window_size1 < 0:
        h_offset2 = off_ht % loop_range1
    elif window_size1 % 2 == 0:
        h_offset2 = (window_size1 - 1) // 2 + (off_ht % 2 == 1)
    else:
        h_offset2 = (window_size1 - 1) // 2

    if window_size2 < 0:
        w_offset2 = off_wt % loop_range2
    elif window_size2 % 2 == 0:
        w_offset2 = (window_size2 - 1) // 2 + (off_wt % 2 == 1)
    else:
        w_offset2 = (window_size2 - 1) // 2

    for rhidx in tl.static_range(loop_range1, loop_range1 *2):
        for rwidx in tl.static_range(loop_range2, loop_range2 *2):
            hidx = (rhidx - ((off_ht - h_offset2) % loop_range1)) % loop_range1
            widx = (rwidx - ((off_wt - w_offset2) % loop_range1)) % loop_range2
            cur_h = off_ht + hidx - h_offset2
            cur_w = off_wt + widx - w_offset2
            for start_n in tl.static_range(0, qs4 // BLOCK_M):
                if cur_h >=0 and cur_h < qs2:
                    if cur_w >=0 and cur_w < qs3:
